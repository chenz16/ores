...
def optimize_switching_angles(Vi, V1_desired=311):
    ...
    try:
        ...
        # Minimize higher harmonics (9th and above)
        for i, Vh in enumerate(harmonics[3:]):  # Start from index 3 (9th harmonic)
        ...
        g.append(V1)
        lbg.append(V1_desired - 0.0025 * V1_desired)
        ubg.append(V1_desired + 0.0025 * V1_desired)
        
        # Hard constraints for 3rd, 5th, and 7th harmonics
        for Vh in harmonics[:3]:  # First three harmonics (3rd, 5th, 7th)
            g.append(Vh)
            lbg.append(-0.001 * V1_desired)  # Stricter bounds for lower harmonics
            ubg.append(0.001 * V1_desired)
        
        # Relaxed constraints for higher harmonics
        for Vh in harmonics[3:]:  # Higher harmonics
            g.append(Vh)
            lbg.append(-0.005 * V1_desired)
            ubg.append(0.005 * V1_desired)
        
        # Angle ordering constraints
        for i in range(num_angles-1):
            g.append(alpha[i+1] - alpha[i])
            lbg.append(0.00)  # 2 degrees in radians
            ubg.append(0.5)  # 15 degrees in radians
...
if __name__ == "__main__":
    # Generate random voltages around 100V with ±5V variation
    mean_voltage = 100.0
