...
float get_module_output_voltage(uint32_t bridge_state, float vdc, float current) {
    ...
    switch (high_side_state) {
        ...
        case 0b00:  // Both off (all switches off)
            // In charging mode (current > 0), diodes conduct
            // In discharging mode (current < 0), high impedance
            return (current > 0.0f) ? -vdc : 0.0f;
        default:
            return 0.0f;
    }
}
...
float simulate_mmc_output(single_dc_source_t * dc_sources, int num_of_modules) {
    ...
    for (int i = 0; i < num_of_modules; i++) {
        ...
        output_voltage += get_module_output_voltage(bridge_state, dc_sources[i].vdc, dc_sources[i].current);
    }
